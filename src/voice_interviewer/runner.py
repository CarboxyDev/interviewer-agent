...
class ConversationRunner:
    ...
    async def run(self, session_id: str) -> None:
        ...
        try:
            ...
            speech_events = self.stt.transcribe(self.audio.candidate_audio())
    ...
    async def _conduct_interview(
        ...
        while not self._stop_requested.is_set():
            ...
            if turn.should_end or remaining <= 0:
                await self.audio.play_bot_audio(self.tts.synthesize(turn.say))
                break
            response = await self._say_and_receive(
                turn.say,
                events,
        ...
        try:
            while True:
                ...
                if event_task in done:
                    ...
                    event_task = None
                    if event.kind is SpeechEventKind.SPEECH_STARTED and not playback.done():
                        await self.audio.stop_bot_audio()
