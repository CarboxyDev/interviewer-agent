...
export interface SampleRole {
  ...
  clarityFollowup: string;
  next: string;
  audio?: string;
  times?: { gap: number[]; strength: number[] };
}
export type RoleId = keyof typeof sampleRoles;
...
} satisfies Record<string, SampleRole>;
...
export function sampleScenario(
  ...
  const role: SampleRole = sampleRoles[roleId];
